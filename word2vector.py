...
def read_data(filename):
    with zipfile.ZipFile(filename) as f:
        # f 为一个zipfile对象， 这里的f.namelist()代表了其中的压缩的文件名称列表(一个zip文件中可以包含多个文件)
...
def dictionary_to_file(dictionary, filename="/home/jack/train_data/word2vector/dict.txt"):
    f = open(filename, "wb")
...
def generate_batch(batch_size,num_skips,skip_window):
    ...
    for i in range(batch_size // num_skips):
        ...
        for j in range(num_skips):
            ...
            batch[i * num_skips + j] = buffer[skip_window]      #语境词汇
            labels[i * num_skips +j,0] = buffer[target]         #目标词汇
...
batch, labels = generate_batch(8, 2, 1)   # 数据的原始输入
...
with graph.as_default():
    ...
    # 选择运行的device为GPU 0  很奇怪，没有使用GPUqueue占用了显存
    with tf.device("/cpu:0"):
        ...
        loss = tf.reduce_mean(tf.nn.nce_loss(
            weights=nce_weights,    # nce weight
            biases=nce_biases,      # nce biases
            labels=train_labels,    # train labels
            inputs=embed,           # nce inputs
            num_sampled=num_sampled,# 随机选择的负样本的噪声单词的数量
...
with tf.Session(graph=graph) as session:
    ...
    saver.save(session, "/home/jack/train_data/word2vector/wv")
